...
{
    internal class CameraMove : IEcsRunSystem
    {
            ...
            foreach (var index in _filter)
            {
                ...
                else
                {
                    if (camera.Get<MoveCameraComponent>().timer < 1)
                    {
                        ...
                        cameraPos.transform.position += camera.Get<MoveCameraComponent>().delta * (Time.deltaTime * camera.Get<MoveCameraComponent>().dragSpeed * camera.Get<MoveCameraComponent>().curve.Evaluate(camera.Get<MoveCameraComponent>().timer));
