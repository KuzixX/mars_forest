...
namespace Client.Scripts.ECS_Feature.Interaction_Features.system
{
    internal class Interaction : IEcsRunSystem, IEcsInitSystem
    {
        ...
        public void Run()
        {
            ...
            if (_lock.IsEmpty())
            {
                foreach (var index in _target)
                {
                    ref var target = ref _target.Get1(index);
                    target.transform.position = interactionData.CellPos;
                } 
            }
        }
    }
